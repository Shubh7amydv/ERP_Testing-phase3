...
import { 
  ...
  Bus, Clock, Video, ArrowRight, Send, AlertCircle
} from 'lucide-react';
...
export type FrontOfficeSubView = 
  ...
  | 'fo-postal-receive' | 'fo-postal-dispatch' | 'fo-phone-call';

export type SendSMSSubView = 
  | 'sms-send-student' | 'sms-send-staff' | 'sms-send-custom' 
  | 'sms-template' | 'sms-log' | 'sms-balance';

export type HostelSubView = 
  | 'hostel-room' | 'hostel-room-type' | 'hostel-allotment' | 'hostel-allotment-list' 
  | 'hostel-fees' | 'hostel-report';

export type EContentSubView = 
  | 'econtent-add' | 'econtent-list' | 'econtent-homework' | 'econtent-homework-submit';
          ...
          <div className="erp-card"><div className="table-container"><table className="erp-table">
            ...
            <tbody>{HR_STAFF.map(s => (
              <tr key={s.id}>
                ...
                <td><div style={{ display: 'flex', gap: 6 }}><button onClick={() => showToast('Staff profile opened for editing')} style={{ padding: '4px 10px', backgroundColor: '#0284c7', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>Edit</button></div></td>
              </tr>
            ))}</tbody>
          </table></div></div>
        </div>
          ...
          <div className="erp-card" style={{ marginBottom: 16 }}>
            <div className="erp-card-body" style={{ padding: 20 }}>
              <div className="form-grid">
                ...
                {subView === 'cert-sports' && (<>
                  <div className="form-group"><label>Achievement / Event *</label><input type="text" placeholder="e.g. State Level Basketball Champion" /></div>
                  <div className="form-group"><label>Achievement Date *</label><input type="date" /></div>
                </>)}
...
// ===========================================================
// ACADEMIC MODULE (Subject, Lesson Plan, Homework, Syllabus)
// ===========================================================
const HOMEWORK_LIST = [
...
export function AcademicModule({ initialSubView = 'academic-subject', onNavigateSubView }: { initialSubView?: AcademicSubView; onNavigateSubView?: (sv: AcademicSubView) => void }) {
  ...
  return (
    <div>
      ...
      {/* SUBJECT / CHAPTER / LESSON PLAN / SYLLABUS */}
...
export function FrontOfficeModule({ initialSubView = 'fo-enquiry-list', onNavigateSubView }: { initialSubView?: FrontOfficeSubView; onNavigateSubView?: (sv: FrontOfficeSubView) => void }) {
  ...
  return (
    <div>
      ...
      {/* POSTAL / PHONE CALL / FOLLOWUP */}
      {(subView === 'fo-postal-receive' || subView === 'fo-postal-dispatch' || subView === 'fo-phone-call' || subView === 'fo-followup') && (
        <SimpleConfigView
          title={subView === 'fo-postal-receive' ? 'Postal Receive Register' : subView === 'fo-postal-dispatch' ? 'Postal Dispatch Register' : subView === 'fo-phone-call' ? 'Phone Call Log Register' : 'Enquiry Follow-Up Tracker'}
          items={subView === 'fo-postal-receive' ? ['CBSE Circular #2026-45 (Received: 18-Jul)', 'NCERT Book Delivery (Received: 19-Jul)', 'Staff Appointment Letter (Received: 20-Jul)'] : subView === 'fo-postal-dispatch' ? ['TC Letter for Rahul Sharma (Sent: 17-Jul)', 'Parent Invitation for PTA (Sent: 20-Jul)'] : subView === 'fo-phone-call' ? ['Parent call - Rohan Patel (10:30 AM)', 'CBSE Helpdesk (02:15 PM)', 'Supplier Quote (03:40 PM)'] : ['Rajesh Kumar - Class 6 Admission (Due: 22-Jul)', 'Sunita Sharma - Fee Structure (Due: 24-Jul)']}
          onAdd={() => showToast('Entry added!')}
        />
      )}

      {subView === 'fo-enquiry-add' && (
        <div className="erp-card">
          <div className="erp-card-body" style={{ padding: 20 }}>
            <p style={{ color: '#475569', fontSize: 13 }}>Use the Enquiry List view to add a new enquiry.</p>
            <button onClick={() => { setSubView('fo-enquiry-list'); setShowForm(true); onNavigateSubView?.('fo-enquiry-list'); }} className="erp-btn btn-primary" style={{ backgroundColor: '#00696b' }}>Go to Enquiry List</button>
          </div>
        </div>
      )}
    </div>
...
export function SendSMSModule({ initialSubView = 'sms-send-student', onNavigateSubView }: { initialSubView?: SendSMSSubView; onNavigateSubView?: (sv: SendSMSSubView) => void }) {
  ...
  return (
    <div>
      ...
      {/* SMS TEMPLATE */}
      {subView === 'sms-template' && (
        <SimpleConfigView
          title="SMS Template Manager"
          items={['Fee Reminder: "Dear Parent, fee for [MONTH] is due. Amount: ₹[AMOUNT]. Please pay by [DATE]."', 'Attendance Alert: "Your ward [NAME] was absent on [DATE]. Please contact school."', 'PTM Invite: "PTM is scheduled on [DATE] at [TIME]. Please attend."', 'Exam Schedule: "Exam for [SUBJECT] on [DATE] at [TIME]. Bring admit card."']}
          onAdd={() => showToast('Template saved!')}
        />
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      {subView === 'payroll-generate' && (
        <div>
          ...
          <div className="erp-card"><div className="table-container"><table className="erp-table">
            ...
            <tfoot><tr style={{ backgroundColor: '#f8fafc', fontWeight: 800 }}>
              <td colSpan={6}>TOTAL</td>
              <td>{fmt(PAYROLL_DATA.reduce((s, p) => s + p.gross, 0))}</td>
              <td style={{ color: '#dc2626' }}>{fmt(PAYROLL_DATA.reduce((s, p) => s + p.pf, 0))}</td>
              <td style={{ color: '#dc2626' }}>{fmt(PAYROLL_DATA.reduce((s, p) => s + p.tds, 0))}</td>
              <td style={{ color: '#16a34a' }}>{fmt(PAYROLL_DATA.reduce((s, p) => s + p.net, 0))}</td>
            </tr></tfoot>
          </table></div></div>
        </div>
      )}

      {/* SALARY SLIP / PF-ESI / ADVANCE / REPORT */}
      {(subView === 'payroll-slip' || subView === 'payroll-pf-esi' || subView === 'payroll-advance' || subView === 'payroll-report') && (() => {
        ...
        const payrollTitle = subView === 'payroll-slip' ? 'Salary Slip Generator' : subView === 'payroll-pf-esi' ? 'PF / ESI Contribution Register' : subView === 'payroll-advance' ? 'Staff Advance Salary Requests' : 'Payroll Summary Report';
        return <SimpleConfigView title={payrollTitle} items={items} onAdd={() => showToast('Action performed!')} />;
      })()}
    </div>
  );
}
    ...
    <div>
      ...
      {(subView === 'lib-member' || subView === 'lib-report') && (
        <SimpleConfigView
          ...
          items={subView === 'lib-member' ? ['Aarav Sharma (Class 10-A) - Member Since: 01-Jul-2026', 'Aditi Verma (Class 10-A) - Member Since: 01-Jul-2026', 'Ms. Seema Solanki (Staff) - Member Since: 01-Apr-2022'] : ['Total Books: 53 Titles | 285 Copies', 'Books Issued This Month: 42', 'Books Returned This Month: 38', 'Books Overdue: 4', 'Total Fine Collected: ₹240']}
...
export function MasterModule({ initialSubView = 'master-class', onNavigateSubView }: { initialSubView?: MasterSubView; onNavigateSubView?: (sv: MasterSubView) => void }) {
  ...
  const masterTitles: Record<MasterSubView, string> = {
    ...
    'master-id-card': 'ID Card Template Designer', 'master-gallery': 'School Photo Gallery Manager',
  };
  ...
  return (
    <div>
      ...
      <div className="view-header">
        <div>
          <h2 className="view-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Settings size={22} color="#00696b" /> Master Configuration & Setup</h2>
          <span className="view-subtitle">School master data: classes, sections, subjects, sessions, categories & ID card templates</span>
        </div>
      </div>
